...
def AutoQ(root, stR, semaphore, lock, counter):
    with semaphore:
        ...
        p = subprocess.run(f'timeout {TIMEOUT} ../../build/cli/autoq {root}/pre.aut {root}/circuit.qasm {root}/spec.aut {root}/constraint.txt', shell=True, capture_output=False, executable='/bin/bash')
        ret = p.returncode
        if ret == 0:
