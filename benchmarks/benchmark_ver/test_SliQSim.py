...
if 'MCToffoli' in root:
    ...
    for i in range(p2n):
        ...
        text_file.write(qc)
        text_file.close()
        p = subprocess.run(f'{EXE} --sim_qasm /tmp/{root2}/circuit.qasm --type 1 --print_info', shell=True, capture_output=True, executable='/bin/bash')
    ...
    n = int(re.findall('[0-9]+', root)[-1]) # half of the number of qubits (the number of solution tabs)
    with open(f'{root}/circuit.qasm', 'r') as file:
        data = file.read()
    total_time = 0
    p2n = pow(2, n)
    ...
    p = subprocess.run(f'{EXE} --sim_qasm {root}/circuit.qasm --type 1 --print_info', shell=True, capture_output=True, executable='/bin/bash')
    # Assume the 1st line format - Runtime: 0.013964 seconds
    # print(p.stdout.decode('utf-8').strip())
    tmp = list(map(lambda x: x.decode('utf-8'), p.stdout.splitlines()))[0].strip()
    total_time = float(tmp.split(' ')[1])
