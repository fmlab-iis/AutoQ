...
def symqv(root, semaphore, lock, counter):
    with semaphore:
        ...
        data['q'] = q
        data['G'] = G
        cmd = f"ulimit -v {TOTAL_MEMORY//NUM_OF_THREADS} && cd /home/guest/fabianbauermarquart-symqv-fa8ec7f/POPL25/ && timeout {TIMEOUT} ./{symqvMap[root.split('/')[1]]}.py /home/guest/AutoQ/POPL25/Kick-the-Tires/non-parameterized/{sys.argv[1]}/{root.split('/')[1]}/{root.split('/')[2]}/circuit.qasm"#; print(cmd)
        ...
        if ret == 124:
            ...
            # assume format: ('', {}, 8.522298574447632) => OOM
            if re.search(r"\('',.*, ([\d\.]*)\)", p.stdout.decode('utf-8').splitlines()[-1].strip()):
                data['total'] = str(round(end - begin, 1))
                data['result'] = 'ERROR'
            else:
                ...
                if match:
                    ...
                    print(symqv.__name__, root, p.stdout.decode('utf-8').splitlines())
                    data['total'] = str(round(end - begin, 1))
                    data['result'] = 'X'
        lock.acquire()
        ##############################################
        ...
        if key in CaALMap:
            ...
            if ret == 124:
                ...
                if (('correct' in sys.argv[1]) and (answer == 'Valid')) or (('correct' not in sys.argv[1]) and (answer == 'Invalid')):
                    ...
                    print(CaAL.__name__, root, p.stdout.decode('utf-8').splitlines())
                    data['total'] = str(round(end - begin, 1))
                    data['result'] = 'X'
        else:
            data['total'] = str(TIMEOUT)
