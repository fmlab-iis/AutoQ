...
def CTA(root, stR, semaphore, lock, counter):
    with semaphore:
        ...
        cmd = f'timeout {TIMEOUT} {EXE} eq {root} ../origin/{root.split("qasm", 1)[0] + "qasm"} --latex'#; print(cmd)
        begin = time.time()
        ...
        if ret == 0:
            ...
            stR.value += str(round(end - begin, 2)) + ' & ' + 'TIMEOUT'
        else:
            stR.value += str(round(end - begin, 2)) + ' & ' + 'ERROR'
        data = dict()
