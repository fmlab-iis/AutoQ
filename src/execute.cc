...
bool AUTOQ::Automata<Symbol>::execute(const std::string& filename, std::vector<int> qubit_permutation, const std::vector<AUTOQ::Automata<Symbol>> &loopInvariants) {
    return execute(filename.c_str(), qubit_permutation, loopInvariants);
}
template <typename Symbol>
bool AUTOQ::Automata<Symbol>::execute(const char *filename, std::vector<int> qubit_permutation, const std::vector<AUTOQ::Automata<Symbol>> &loopInvariants) {
    initialize_stats();
