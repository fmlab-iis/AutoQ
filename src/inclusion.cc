...
#define INCLUSION_DEBUG(msg) if (inclusion_debug) AUTOQ_DEBUG(msg)
...
bool nonscaled_inclusion_with_or_without_renaming(AUTOQ::SymbolicAutomata autA, AUTOQ::SymbolicAutomata autB, bool renaming) {
    if (AUTOQ::String::trim(autA.constraints).empty()) autA.constraints = "true";
    if (AUTOQ::String::trim(autB.constraints).empty()) autB.constraints = "true";
    autB = autB.operator||(AUTOQ::SymbolicAutomata::zero_amplitude(autB.qubitNum));
    // autA.k_unification(); autB.k_unification(); // TODO: is it necessary for LSTA?
                ...
                if (is_leaf_vertex) { // only when considering some particular transitions
                    ...
                    for (const auto &leaf_pairs_of_one_new_cell : leaf_pairs_of_one_new_vertex) {
                        ...
                        for (const auto &pair : leaf_pairs_of_one_new_cell) {
                            ...
                            if (arguments.empty())
                                assertion += "(- (* 1 F" + std::to_string(counter) + "r) (* 0 F" + std::to_string(counter) + "i)))\n";
                            else
                                assertion += "(- (* 1 (F" + std::to_string(counter) + "r" + arguments + ")) (* 0 (F" + std::to_string(counter) + "i" + arguments + "))))\n";
                            assertion += "(distinct ";
                            assertion += "E" + std::to_string(counter) + "i ";
                            if (arguments.empty())
                                assertion += "(+ (* 1 F" + std::to_string(counter) + "i) (* 0 F" + std::to_string(counter) + "r)))\n";
                            else
                                assertion += "(+ (* 1 (F" + std::to_string(counter) + "i" + arguments + ")) (* 0 (F" + std::to_string(counter) + "r" + arguments + "))))\n";
                            counter++;
                        }
                        assertion += ")\n"; // "(or "
                    }
...
bool AUTOQ::SymbolicAutomata::operator<<=(AUTOQ::SymbolicAutomata autB) const {
    return nonscaled_inclusion_with_or_without_renaming(*this, autB, false);
}
template <>
