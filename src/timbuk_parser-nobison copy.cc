	...
	for (const std::string& line : lines)
	{
		...
		if (!are_transitions)
		{
			...
			else if ("States" == first_word)
			{
				...
				while (!str.empty())
				{
                    ...
                    if (!result.stateNameToId.contains(state_num.first)) {
		...
		{	// processing transitions
			...
			{	// contains a tuple of states
                ...
                std::cout << str_state_tuple << "\n";
				std::vector<std::string> state_vectoR = split_delim(str_state_tuple, ',');
				std::vector<TreeAutomata::State> state_vector;
                for (std::string& state : state_vectoR)
				{
					state = trim(state);
				...
				if ((state_vectoR.size() == 1) && state_vectoR[0] == "")
				{
					state_vectoR = { };
                    state_vector = { };
				}
