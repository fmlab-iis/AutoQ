...
if 'MCToffoli' in root:
    ...
    for i in range(p2n):
        ...
        text_file.write(qc)
        text_file.close()
        p = subprocess.run(f'/home/alan23273850/SliQSim/SliQSim --sim_qasm /tmp/{root2}/circuit.qasm --type 1 --print_info', shell=True, capture_output=True, executable='/bin/bash')
    ...
    p = subprocess.run(f'/home/alan23273850/SliQSim/SliQSim --sim_qasm {root}/circuit.qasm --type 1 --print_info', shell=True, capture_output=True, executable='/bin/bash')
    # Assume the 1st line format - Runtime: 0.013964 seconds
    tmp = list(map(lambda x: x.decode('utf-8'), p.stdout.splitlines()))[0].strip()
    total_time = float(tmp.split(' ')[1])
    if total_time >= 60:
