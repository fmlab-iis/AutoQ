...
for root, dirnames, filenames in sorted(os.walk('.')):
    if ('Random' not in root) and ('Feynman' not in root) and ('RevLib' not in root): continue
    if len(dirnames) == 0 and 'post.aut' in filenames:
        ...
        if timeout1 in root:
            ...
            ret = os.system(f'VATA_PATH=/home/alan23273850/libvata/build/cli/vata timeout 720 ../permutation {root}/pre.aut {root}/bug.qasm /tmp/automata.aut {root}/post.aut')
            ret = (ret >> 8) & 0xff
        if ret == 0:
        ...
        if timeout2 in root:
            ...
            ret = os.system(f'VATA_PATH=/home/alan23273850/libvata/build/cli/vata timeout 720 ../composition {root}/pre.aut {root}/bug.qasm /tmp/automata.aut {root}/post.aut')
            ret = (ret >> 8) & 0xff
        if ret == 0:
            print('', end=' & ', flush=True)
        elif ret == 124:
        ...
        if timeout4 in root:
            ...
            ret = os.system(f'timeout 720 /home/alan23273850/feynman/feynver {root}/circuit.qc {root}/bug.qc')
            ret = (ret >> 8) & 0xff
        if ret == 124:
            if ('Feynman' not in root) and ('RevLib' not in root):
