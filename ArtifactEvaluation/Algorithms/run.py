...
def Feynman(root, str3, semaphore):
    with semaphore:
        p = subprocess.run(f'timeout 720 /home/alan23273850/feynman/feynver {root}/circuit.qc {root}/opt.qc', shell=True, capture_output=True, executable='/bin/bash')
        ret = p.returncode
        if ret == 124:
...
os.system('pkill -9 -f permutation; pkill -9 -f composition; pkill -9 -f SliQSim; pkill -9 -f feynver; pkill -9 -f autoq')
