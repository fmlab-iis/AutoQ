...
def sum_plus_separated_values(value_str):
    ...
    for part in parts:
        if not part:
            continue
        if part == "ERROR" or "ERROR" in part:
            return "ERROR"
        ...
        for tool_path in tool_list_sorted:
            ...
            if "/" in ver:
                try:
                    ...
                    total = (
                        "ERROR"
                        if ("ERROR" in trans or "ERROR" in ver)
                        else "TIMEOUT"
                    )
            else:
                try:
            ...
            if "ERROR" in trans or "ERROR" in ver or "ERROR" in str(total):
                trans, ver, total = "ERROR", "ERROR", "ERROR"
            elif (
                "TIMEOUT" in trans
                or "TIMEOUT" in ver
                or trans == "---"
        ...
        for group_idx, bench_name in enumerate(names):
            ...
            for row_idx, row in enumerate(group_rows):
                ...
                if this_triplet == ["TO", "TO", "TO"]:
                    ...
                    parts.append(text((x_edges[3] + x_edges[6]) / 2, (row_top + row_bottom) / 2, "ERROR", size=18))
                else:
                ...
                if cav23_triplet == ["TO", "TO", "TO"]:
                    ...
                    parts.append(text((x_edges[6] + x_edges[9]) / 2, (row_top + row_bottom) / 2, "ERROR", size=18))
                else:
