...
struct AUTOQ::Complex::nTuple : AUTOQ::Util::mapped_vector<boost::multiprecision::cpp_int> {
    ...
    boost::multiprecision::cpp_int kk = 0;
    ...
    friend std::ostream& operator<<(std::ostream& os, const nTuple& obj) {
        os << AUTOQ::Util::Convert::ToString(static_cast<std::map<long long, boost::multiprecision::cpp_int>>(obj)) << " / sqrt2^" << obj.kk;
        return os;
    }
    ...
    nTuple operator*(const nTuple &o) const {
        ...
        symbol.kk = this->kk + o.kk; // remember to set kk
        return symbol;
    }
    ...
    AUTOQ::Complex::FiveTuple toFiveTuple() const {
        AUTOQ::Complex::FiveTuple fivetuple;
        for (const auto &kv : *this) {
            // k / N = ?? / 4, ?? = 4 * k / N
            if ((kv.first * 4) % N == 0)
                fivetuple.at(kv.first * 4 / N) = kv.second;
            else
                THROW_AUTOQ_ERROR("This number cannot be converted to a FiveTuple.");
        }
        fivetuple.at(4) = kk;
        return fivetuple;
    }
    boost::rational<boost::multiprecision::cpp_int> to_rational() const { // TODO: limited support
        // if (N != 1 && N != 2 && N != 4) THROW_AUTOQ_ERROR("to_rational() is only supported when N = 1, 2, or 4.");
        // auto fivetuple = toFiveTuple();
        return toFiveTuple().to_rational();
    }
    z3::expr realToSMT(z3::context &ctx) const { // TODO: limited support
        if (N != 1 && N != 2 && N != 4) THROW_AUTOQ_ERROR("realToSMT(...) is only supported when N = 1, 2, or 4.");
        auto fivetuple = toFiveTuple();
        return fivetuple.realToSMT(ctx);
    }
    std::string realToSMT() const {
        z3::context ctx;
        return realToSMT(ctx).to_string();
    }
    z3::expr imagToSMT(z3::context &ctx) const { // TODO: limited support
        if (N != 1 && N != 2 && N != 4) THROW_AUTOQ_ERROR("imagToSMT(...) is only supported when N = 1, 2, or 4.");
        auto fivetuple = toFiveTuple();
        return fivetuple.imagToSMT(ctx);
    }
    ...
    double abs2() const {
        double denominator = std::pow(std::sqrt(2.0), static_cast<double>(kk));
    ...
    bool operator<(const auto &rhs) const {
        return kk < rhs.kk ||
              (kk == rhs.kk && AUTOQ::Util::mapped_vector<boost::multiprecision::cpp_int>::operator<(rhs));
    }
    bool operator==(const auto &rhs) const {
        return kk == rhs.kk && AUTOQ::Util::mapped_vector<boost::multiprecision::cpp_int>::operator==(rhs);
    }
    void increase_k() { adjust_k(1); }
    ...
    boost::multiprecision::cpp_int& k() { return kk; }
    const boost::multiprecision::cpp_int& k() const { return kk; } // const version
    void back_to_zero() { clear(); }

private:
    nTuple binary_operation(nTuple o, bool add) const {
        auto This = *this;
        while (This.kk < o.kk) { This.increase_k(); }
        while (This.kk > o.kk) { o.increase_k(); }

        nTuple result = This; // kk also automatically set
        for (const auto &kv : o) {
